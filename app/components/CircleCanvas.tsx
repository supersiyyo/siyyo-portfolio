...
class WaveEffect {
  ...
  draw(ctx: CanvasRenderingContext2D) {
    ...
    const lineWidth = (this.life / this.maxLife) * 4;
...
class Circle {
  ...
  constructor(x: number, y: number, personality?: Personality) {
    ...
    switch (this.personality) {
      case 'playful':
        this.targetColor = { h: 50, s: 100, l: 60 };
        break;
      case 'shy':
        this.targetColor = { h: 180, s: 80, l: 70 };
        break;
      case 'loner':
        this.targetColor = { h: 260, s: 80, l: 40 };
        break;
      case 'social':
        this.targetColor = { h: 140, s: 80, l: 50 };
        break;
    }
  ...
  hyperRage() {
    ...
    this.hyperRagedUntil = Date.now() + 3000;
  }
...
const CircleCanvas: React.FC = () => {
  ...
  useEffect(() => {
    ...
    let circles: Circle[] = [];
    const waveEffects: WaveEffect[] = [];
    const lineEffects: LineEffect[] = [];
    const stardust: StardustParticle[] = [];
    const respawnQueue: { personality: Personality; respawnTime: number }[] = [];
    ...
    const handleClick = (event: MouseEvent) => {
      ...
      circles.forEach((circle) => {
        if (Math.hypot(circle.x - event.clientX, circle.y - event.clientY) < 150) circle.enrage(event.clientX, event.clientY);
      });
    };
      ...
      for (let i = 0; i < circles.length; i++) {
        for (let j = i + 1; j < circles.length; j++) {
          ...
          if (dist < c1.radius + c2.radius) {
            ...
            const v1 = { x: c1.dx, y: c1.dy };
            const v2 = { x: c2.dx, y: c2.dy };
            const m1 = c1.mass;
            const m2 = c2.mass;
            const nx = (c2.x - c1.x) / dist;
            const ny = (c2.y - c1.y) / dist;
